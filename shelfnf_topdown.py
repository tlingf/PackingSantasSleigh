...
def getData():  
    ...
    data = pd.read_table('/Users/ling/PackingSantasSleigh/input/presents.csv', sep=',')  
    #print data  
...
def shelfNF(data):  
    ...
    for i in range(numPresents):  
         ...
         if (xs + presents[i,0] > width + 1): # width + 1 because 1 is the starting point not 0
         ...
         if (ys + presents[i,1] > length + 1):  
              ...
              numInShelf = 0  
              tempPresentHeightShelf = []  
         presentCoordinates[i,0] = data[i,0]  # presentId (confusing b/c presents[i,0] is the width)
         presentCoordinates[i,[1,7,13,19]] = xs  # starting width
         presentCoordinates[i,[4,10,16,22]] = xs + presents[i,0] - 1  # increase the width
         presentCoordinates[i,[2,5,14,17]] = ys  
         presentCoordinates[i,[8,11,20,23]] = ys + presents[i,1] - 1  # increase the length
         presentCoordinates[i,[3,6,9,12]] = zs  
         presentCoordinates[i,[15,18,21,24]] = zs - presents[i,2] + 1  # move down in height
         xs = xs + presents[i,0] # width
         numInRow = numInRow + 1  
...
def saveCSV(predictions):  
    datafile = pd.read_table('/Users/ling/PackingSantasSleigh/input/presents.csv', sep=',')  
    ...
    submission.to_csv('sample_topdown.csv', index = False)  
